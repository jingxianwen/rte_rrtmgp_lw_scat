...
program rte_rrtmgp_clouds
  ...
  use mo_garand_atmos_io,    only: read_atmos, write_lw_fluxes, write_sw_fluxes, &
                               write_lw_optics,write_sw_optics,write_cloud_props !U-MICH added.
  use mo_ice_optics_mc6,     only: flag_mc6 !U-MICH added.
  use mo_mc6_table,          only: mc6_table_init  !U-MICH added.
  implicit none
  ! ----------------------------------------------------------------------------------
  ! Variables
  ! ----------------------------------------------------------------------------------
...
!! U-MICH added, May 19, 2020-->
  real(wp) :: rei_temp  !temperary rei
!! <--
...
!! U-MICH add -->
  logical :: write_optics = .true., write_clouds = .true.
!! <--
...
!! two radiative transfer options (U-MICH add 2str) --> 
    if (.not. flag_mc6) then
    ! use default 1scl radiative transfer
      allocate(ty_optical_props_1scl::atmos)
      allocate(ty_optical_props_1scl::clouds)
      print*,'flag_mc6 = ',flag_mc6
    else 
    ! use 2str radiative transfer with scattering (U-MICH added)
      allocate(ty_optical_props_2str::atmos)
      allocate(ty_optical_props_2str::clouds)
      call mc6_table_init() !load MC6 ice optics table
      print*,'flag_mc6 = ',flag_mc6
    end if
!! <--
  ...
  do ilay=1,nlay
    do icol=1,ncol
!! Restric ice clouds to 100-500 hPa. (U-MICH, May 19, 2020)
      cloud_mask(icol,ilay) = p_lay(icol,ilay) > 100._wp * 100._wp .and. &
                              p_lay(icol,ilay) < 500._wp * 100._wp !.and. &
                              !mod(icol, 3) /= 0
      !
      ! Ice and liquid will overlap in a few layers
      !

!! use ice clouds only (U-MICH, May 19, 2020) -->
    ! units of lwp and iwp: g/m2
      lwp(icol,ilay) = 0._wp
      iwp(icol,ilay) = merge(10._wp,  0._wp, cloud_mask(icol,ilay) .and. t_lay(icol,ilay) < 263._wp)
      rel(icol,ilay) = 0._wp
    ! restrict rei to 10-180 microns from the first to the last colomn. 
      rei_temp=10._wp+real(icol,kind=wp)/real(ncol,kind=wp)*(180._wp-10._wp) 
      rei(icol,ilay) = merge(rei_temp, 0._wp, iwp(icol,ilay) > 0._wp)
!! <--
...
!********************************************
!check optics data type
!    if (is_lw) then
!      select type(atmos)
!        class is (ty_optical_props_1scl)
!          print*,"atmos type is 1scl"      
!        class is (ty_optical_props_2str)
!          print*,"atmos type is 2str"      
!        class default
!          call stop_on_err("rte_rrtmgp_clouds: Don't recognize the kind of optical properties ")
!      end select
!      select type(clouds)
!        class is (ty_optical_props_1scl)
!          print*,"clouds type is 1scl"      
!        class is (ty_optical_props_2str)
!          print*,"clouds type is 2str"      
!          !where(clouds%ssa(:,:,:)>0._wp) clouds%ssa(:,:,:)=0.8_wp 
!          print*,clouds%ssa(1,:,1)
!        class default
!          call stop_on_err("rte_rrtmgp_clouds: Don't recognize the kind of optical properties ")
!      end select
!    end if
...
!! U-MICH write optics -->
    if(write_optics) then 
      select type(clouds)
        class is (ty_optical_props_1scl)
          call write_lw_optics(input_file, clouds%tau)
        class is (ty_optical_props_2str)
          call write_lw_optics(input_file, clouds%tau, clouds%ssa, clouds%g)
        class default 
          call stop_on_err("write_lw_optics: Don't recognize the kind of optical properties ")
      end select
    end if
!! <--
...
!! U-MICH write optics -->
    if(write_optics) then 
      select type(clouds)
        class is (ty_optical_props_1scl)
          call write_sw_optics(input_file, clouds%tau)
        class is (ty_optical_props_2str)
          call write_sw_optics(input_file, clouds%tau, clouds%ssa, clouds%g)
        class default 
          call stop_on_err("write_sw_optics: Don't recognize the kind of optical properties ")
      end select
    end if
!! <--
...
!! U-MICH write clouds -->
  if(write_clouds) call write_cloud_props(input_file, lwp, iwp, rel, rei)
!! <--
