...
contains
   ...
   !----------------------------------------------------------------------------
   ! U-MICH added, computing MC6 cloud LW scattering properties 
   subroutine get_cloud_optics_lw_scat(state, pbuf, optics_out)
      ...
      ! U-MICH added. -->
      real(r8), dimension(nlwbands,pcols,pver) :: &
            ice_tau_ssa, liq_tau_ssa, snow_tau_ssa, cloud_tau_ssa, combined_tau_ssa, &
            ice_tau_ssa_g, liq_tau_ssa_g, snow_tau_ssa_g, cloud_tau_ssa_g, combined_tau_ssa_g, &
            ice_tau_ssa_f, liq_tau_ssa_f, snow_tau_ssa_f, cloud_tau_ssa_f, combined_tau_ssa_f 
      ...
      snow_tau_ssa(:,:,:) = 0.0
      cloud_tau_ssa(:,:,:) = 0.0
      combined_tau_ssa(:,:,:) = 0.0
      ice_tau_ssa_g(:,:,:) = 0.0
      liq_tau_ssa_g(:,:,:) = 0.0
      snow_tau_ssa_g(:,:,:) = 0.0
      cloud_tau_ssa_g(:,:,:) = 0.0
      combined_tau_ssa_g(:,:,:) = 0.0
      ice_tau_ssa_f(:,:,:) = 0.0
      liq_tau_ssa_f(:,:,:) = 0.0
      snow_tau_ssa_f(:,:,:) = 0.0
      cloud_tau_ssa_f(:,:,:) = 0.0
      combined_tau_ssa_f(:,:,:) = 0.0

      ! Get ice optics
      !call get_ice_optics_lw(state, pbuf, ice_tau)  ! U-MICH commented
      ! U-MICH added. -->
      call get_ice_optics_lw_scat(state, pbuf, ice_tau, ice_tau_ssa, &
                                  ice_tau_ssa_g, ice_tau_ssa_f)
      ...
      cloud_tau = liq_tau + ice_tau
      ! U-MICH added. -->
      cloud_tau_ssa = liq_tau_ssa + ice_tau_ssa
      cloud_tau_ssa_g = liq_tau_ssa_g + ice_tau_ssa_g
      ...
      ! U-MICH added. -->
      call combine_properties(nlwbands, ncol, pver, &
         cloud_fraction(1:ncol,1:pver), cloud_tau_ssa(1:nlwbands,1:ncol,1:pver), &
         snow_fraction(1:ncol,1:pver), snow_tau_ssa(1:nlwbands,1:ncol,1:pver), &
         combined_tau_ssa(1:nlwbands,1:ncol,1:pver) &
      )
      call combine_properties(nlwbands, ncol, pver, &
         cloud_fraction(1:ncol,1:pver), cloud_tau_ssa_g(1:nlwbands,1:ncol,1:pver), &
         snow_fraction(1:ncol,1:pver), snow_tau_ssa_g(1:nlwbands,1:ncol,1:pver), &
         combined_tau_ssa_g(1:nlwbands,1:ncol,1:pver) &
      )
      ...
      ! Check values
      !call assert_range(optics_out%optical_depth, 0._r8, 1e20_r8, &   ! U-MICH commented.
      !                  'get_cloud_optics_lw: optics_out%optical_depth') 
      ! U-MICH added. -->
      call assert_range(optics_out%optical_depth, 0._r8, 1e20_r8, &
                        'get_cloud_optics_lw_scat: optics_out%optical_depth')
      call assert_range(optics_out%single_scattering_albedo, 0._r8, 1._r8, &
                        'get_cloud_optics_lw_scat: optics_out%single_scattering_albedo')
      call assert_range(optics_out%assymmetry_parameter, -1._r8, 1._r8, &
                        'get_cloud_optics_lw_scat: optics_out%assymmetry_parameter')

   end subroutine get_cloud_optics_lw_scat
   ...
   subroutine set_cloud_optics_lw_scat(state, pbuf, kdist, optics_out)
      ...
      type(physics_buffer_desc), pointer :: pbuf(:)
      type(ty_gas_optics_rrtmgp), intent(in) :: kdist
      !type(ty_optical_props_1scl), intent(inout) :: optics_out !U-MICH commented.
      !U-MICH added. -->
      type(ty_optical_props_2str), intent(inout) :: optics_out
      ...
      ! g-point, rather than by band, and subcolumn routines will associate each
      ! g-point with a stochastically-sampled cloud state
      ! call handle_error(optics_out%alloc_1scl(ncol, nlev_rad, kdist)) !U-MICH commented.
      ! U-MICH added.
      call handle_error(optics_out%alloc_2str(ncol, nlev_rad, kdist))
      call optics_out%set_name('longwave cloud optics')

      ! Get cloud optics using CAM routines. This should combine cloud with snow
      ! optics, if "snow clouds" are being considered
      call optics_cam%initialize(nlwbands, ncol, pver)
      !call get_cloud_optics_lw(state, pbuf, optics_cam)  ! U-MICH commented.
      ! U-MICH added.
      call get_cloud_optics_lw_scat(state, pbuf, optics_cam)
